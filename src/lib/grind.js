...
export const MIN_UNIT_PRICE = CA_MINIMUM_WAGE; // derived — keep for backward compat
export const MAX_UNIT_PRICE = 500;   // per-hour rate, flat price, and total charge cap
export const MAX_TOTAL_PRICE = 2000; // entity schema bound (loose; server enforces MAX_UNIT_PRICE on totals)
...
export const CATEGORY_RECOMMENDED_RANGES = {
  ...
  tech_help:   { FIXED: { min: 17, max: 50 }, HOURLY: { min: 17, max: 40 } },
  lawn_care:   { FIXED: { min: 25, max: 60 }, HOURLY: { min: 20, max: 35 } },
  car_washing: { FIXED: { min: 20, max: 50 }, HOURLY: { min: 20, max: 30 } },
  odd_jobs:    { FIXED: { min: 20, max: 75 }, HOURLY: { min: 17, max: 30 } },
  pet_sitting: { FIXED: { min: 17, max: 40 }, HOURLY: { min: 17, max: 25 } },
};
...
export function computeFees(total) {
  const platform_fee = Math.round((total * PLATFORM_FEE_RATE + PLATFORM_FEE_FIXED) * 100) / 100;
