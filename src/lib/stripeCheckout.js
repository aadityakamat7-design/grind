...
export async function startCheckout(bookingId) {
  const res = await base44.functions.invoke("createCheckout", { bookingId, origin: window.location.origin });
  const { url, paid } = res.data || {};
  if (paid) return { paid: true };
  if (window.self !== window.top) {
    alert("Checkout only works from the published app. Open the app in a new tab to complete payment.");
    return { blocked: true };
  }
