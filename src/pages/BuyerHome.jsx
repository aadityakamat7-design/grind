...
export default function BuyerHome() {
  ...
  return (
    ...
    <div className="space-y-7">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Hi, {(user.full_name || "neighbor").split(" ")[0]} 👋</h1>
        <p className="text-sm text-muted-foreground mt-1">Trusted teen help, right in your neighborhood.</p>
      </div>

      <Link to="/browse" className="flex items-center gap-3 bg-foreground rounded-2xl p-4 text-background shadow-card hover:opacity-90 transition-opacity">
        <div className="w-10 h-10 rounded-xl bg-background/20 flex items-center justify-center shrink-0">
          <Search className="w-5 h-5" />
        </div>
        <div className="flex-1">
          <p className="font-semibold text-sm">Browse & book local teens</p>
      ...
      <div>
        ...
        {upcoming.length === 0 ? (
          <p className="text-sm text-muted-foreground">Nothing booked yet — browse local teens and set up your first appointment.</p>
        ) : (
          <div className="space-y-3">
      ...
      <div>
        <h2 className="font-semibold text-foreground mb-3">Past appointments</h2>
        {past.length === 0 ? (
          <p className="text-sm text-muted-foreground">Your completed jobs will show up here.</p>
        ) : (
          <div className="space-y-3">
            {past.map((b) => (
              <div key={b.id} className="space-y-1.5">
                ...
                {b.status === "completed" && (
                  <Link to={`/teens/${b.teen_user_id}`} className="inline-flex items-center gap-1 text-xs font-medium text-foreground hover:underline ml-1">
