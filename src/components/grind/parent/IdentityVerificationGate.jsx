...
// Inline identity-verification gate shown the first time a parent tries to
// approve/deny a booking. Launches Stripe Identity, handles the redirect
// return, and calls onVerified once the parent is confirmed.
export default function IdentityVerificationGate({ open, onOpenChange, onVerified }) {
  ...
  // If we just returned from Stripe Identity, auto-check the result.
  useEffect(() => {
    if (!open) return;
