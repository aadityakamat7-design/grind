...
// Shows the parent a running total of their teen's hours worked this week
// against the legal limit for their age and state.
export default function WeeklyHoursCard({ teenName, teenState, teenAge, bookings }) {
  if (teenAge == null || !teenState) return null;
  ...
  const hoursThisWeek = activeBookings.reduce((sum, b) => {
    const estimated = b.estimated_hours || 2;
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
      <div className="flex items-center justify-between mb-2">
        <p className="font-bold text-slate-900 text-sm flex items-center gap-1.5">
          <Clock className="w-4 h-4 text-blue-500" /> {teenName}'s hours this week
        </p>
        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
          exceeded ? "bg-rose-100 text-rose-700" : approaching ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"
        }`}>
      ...
      <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${
            exceeded ? "bg-rose-500" : approaching ? "bg-amber-500" : "bg-emerald-500"
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {(approaching || exceeded) && (
          <p className="text-[11px] font-semibold flex items-center gap-1 text-amber-600">
      ...
      <p className="text-[10px] text-slate-400 mt-1.5">
        ...
        {rules.prohibitedDuringSchoolHours && ` · No work during school hours`}
      </p>
    </div>
  );
}