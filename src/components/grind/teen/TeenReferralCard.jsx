...
// Same two-sided referral program as neighbors get, but rewards land in the
// teen's Grind Wallet as cash instead of a booking credit.
export default function TeenReferralCard({ profile }) {
  ...
  useEffect(() => {
    if (!profile) return;
    (async () => {
      const p = await ensureReferralCode(profile, "TeenProfile");
  ...
  const copy = () => {
    navigator.clipboard.writeText(`Join me on Kickstart — trusted teen jobs in our neighborhood! Use my code ${code} when you sign up and we both get ${money(REFERRAL_REWARD)}: ${window.location.origin}`);
    setCopied(true);
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Invite a friend — when they complete their first job or booking, you both get {money(REFERRAL_REWARD)} added straight to your Grind Wallet.
          </p>
              ...
              {referrals.map((r) => (
                <div key={r.id} className="flex justify-between text-slate-500">
                  ...
                  <span className={r.status === "completed" ? "text-emerald-600 font-bold" : "text-amber-600"}>
                    {r.status === "completed" ? `+${money(r.reward_amount || REFERRAL_REWARD)}` : "First job pending"}
                  </span>
                </div>
              ))}
              {earned > 0 && (
